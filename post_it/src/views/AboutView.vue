...
<form>
  <p>
    ...
    <input  class="bouton" type="submit" id="envoyer" v-on:click="post()" value="Add new postit"/>
  </p>
</form>
  
<!-- <form>
<input type ="textarea" v-model ="form.title"> 
...
<RouterLink :to="/notes/ +item._id">Afficher</RouterLink>
...
<div  class="widget-notification-postit" > 
    <div class="title-notification-postit">{{item.title}}</div>
	
    <div class="recado-notification-postit">
      {{item.content[0]}}
    </div>
	<button type = "submit" v-on:click="deleteItem(item._id)" > Delete </button>
<RouterLink :to="/notes/ +item._id">Afficher</RouterLink>
...
export default {
  ...
  async mounted()
  {
    let result = await axios.get("http://5.135.119.239:3090/notes");
    console.log(result.data.notes)
    this.list=result.data.notes
  },

  methods: {
    async deleteItem(cardId) {
      await axios.delete("http://5.135.119.239:3090/notes/"+cardId)
      // .then((result)=> {
      //   this.list=result.data.notes
      // })je suis un charo et jm les beurettes
      this.$toast.success(`Success delete`);

    },

    post() {
      axios.post("http://5.135.119.239:3090/notes/", this.form)
      .then(function(response){
        this.$toast.show(`Success add`);

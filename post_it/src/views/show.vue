<template>
<!-- //POSTIT LOCAL STORAGE -->
<div class="app">
  <h1>Notes</h1> <button @click="createNote" class="create-new-button">Create new</button>
  <div class="notes">
    <nav>
      <p v-if="!notes.length">No notes saved</p>
      <ul v-if="notes.length">
        <li v-for="note in notes">
          <button @click="currentNote = note" :class="{active: note === currentNote}">{{note.title}}</button>
        </li>
      </ul>
    </nav>
